...
{{- define "cluster.configmap" -}}
    ...
    {{- /* Strip Helm-only NATS fields after the final merge - these must never reach the Shield binary */ -}}
    {{- $natsConfig := dig "cluster_scanner" "runtime_status_integrator" "nats_server" (dict) $config -}}
    {{- $_ := unset $natsConfig "password" -}}
    {{- $_ := unset $natsConfig "password_existing_secret" -}}
    {{- $config | toYaml -}}
{{- end }}
